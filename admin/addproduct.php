...
<?php
		...
		if (isset($_POST['ADD'])) {
			...
			$sid=$_POST['sbctid'];
		$pname=$_POST['pname'];
		 $chk=mysqli_query($con,"SELECT * FROM `product` WHERE  pname='$pname'");
            if (mysqli_fetch_array($chk))   
             {
                 echo "<script>swal(' Product already exists');</script>";
                }
               else
               {
		$q=mysqli_query($con,"INSERT INTO `product`(`categoryid`, `subid`, `pname`) VALUES ('$cid','$sid','$pname')");
...
<?php } ?>
	...
	<input type="text" name="pname" maxlength="100"  onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123) || event.charCode == 32" class="form-control" placeholder="Enter the Product Name" required>
	</div>

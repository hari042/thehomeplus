...
<?php
		...
		if (isset($_POST['update'])) 
        {
		  ...
		  echo "<script>swal( $suid);
                  window.location='viewsubcat.php';</script>";
		 
          $chk=mysqli_query($con,"SELECT * FROM `subcategory` WHERE subcategoryname='$sname' and `categoryid`='$cid' and `subid`!='$suid'");
            ...
            else
            {
                ...
                $q=mysqli_query($con,"UPDATE `subcategory` SET `subcategoryname`='$sname', `categoryid`='$cid' WHERE `subid`='$suid'");
		      if ($q)
               {
...
$q1=mysqli_query($con,"SELECT * FROM `subcategory` WHERE  subid=$_GET[subcatid]"); 
$r1=mysqli_fetch_array($q1);
$q2=mysqli_query($con,"SELECT * FROM `category` WHERE categoryid=$r1[categoryid]");
...
<form action="" method="post"  autocomplete="off">
	...
	<select name="ctid"  class="form-control" required>
			<option value="<?php echo $r2['categoryid']; ?>"><?php echo $r2['categoryname']; ?></option>
...
<?php } ?>
	...
	<div class="form-group">
	<input type="hidden" name="sid" value="<?php echo $r1['subid']; ?>">
	<label for="uname">Sub Catgeory</label>
	<input type="text" name="subcatname"  maxlength="32" onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123)" class="form-control"  placeholder="Enter the Subcategory Name"  value="<?php echo $r1['subcategoryname']; ?>" required>
	</div>

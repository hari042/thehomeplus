...
<?php
		...
		$scname=$_POST['subcatname'];
	 	$chk=mysqli_query($con,"SELECT * FROM `subcategory` WHERE  subcategoryname='$scname'");
            if (mysqli_fetch_array($chk))   
             {
                 echo "<script>swal(' SubCategory already exists');</script>";
                }
            else
            {
				$q=mysqli_query($con,"INSERT INTO `subcategory` (`categoryid`,`subcategoryname`) VALUES ('$catid','$scname')");
		if ($q) {
...
<?php } ?>
	...
	<input type="text" name="subcatname"  maxlength="32"  onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123 || event.charCode == 32)" class="form-control"  placeholder="Enter the Subcategory Name"  required>
	</div>

	...
	if (isset($_SESSION['uid'])) {
		...
		if ($r=mysqli_fetch_array($q)) 
		{
			...
			if ($r2=mysqli_fetch_array($q2)) 
			{
				...
				$cgst=$total*9/100;
				$sgst=$total*9/100;
				$q3=mysqli_query($con,"UPDATE `cart` SET `qty`='$qty',`price`='$price',`total`='$total',cgst='$cgst',sgst='$sgst' WHERE `cid`='$cartid'");
			}else
			{
				$cgst=$price*9/100;
				$sgst=$price*9/100;
				$q3=mysqli_query($con,"INSERT INTO `cart`(`orderid`, `userid`, `pid`, `qty`, `price`, `total`,`cgst`,`sgst`, `status`) VALUES ('$oid','$uid','$stockid','1','$price','$price','$cgst','$sgst','new')");
			}
			echo '<script>swal({ title: "cart",  text: "item added",  type: "success" }).then(
				function() {
    			window.location = "products.php?subid=$sid";
				});</script>';
...
<div class="page-head_agile_info_w3l">
		<div class="container">
			...
			$qsql = mysqli_query($con,"SELECT * FROM `subcategory` WHERE subid='$sid'");
			$sat = mysqli_fetch_array($qsql); 
			$sqsl = mysqli_query($con,"SELECT * FROM `category` where categoryid='$sat[categoryid]'");
						$cat = mysqli_fetch_array($sqsl);
			?>
				 ...
				 <div class="services-breadcrumb">
						<div class="agile_inner_breadcrumb">
						   ...
						   <ul class="w3_short">
								...
								<li><?php echo $cat['categoryname']; ?></li>
							</ul>
						 </div>
				</div>
	   <!--//w3_short-->
...
<?php 
		$sid=$_GET['subid'];
						$qp = mysqli_query($con,"SELECT * FROM `product` where subid='$sid'");
								...
								<div class="men-pro-item simpleCart_shelfItem">
									...
									<div class="item-info-product ">
										...
										<div class="info-product-price">
											<span class="item_price">₹​<?php  echo $rs['price'];  ?></span><span class="item_price"> in stock <?php if (isset($_SESSION['uid']) && $rs['qty']!=0) { echo $rs['qty']-$qty1; } else { echo $rs['qty']; } ?></span>											<!-- <del><?php  //echo $rp['pname'];  ?></del> -->
										</div>

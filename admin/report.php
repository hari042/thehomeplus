...
$q=mysqli_query($con,"SELECT * FROM  `order` inner join cart on order.orderid=cart.orderid where order.date between '$_POST[date1]' and '$_POST[date2]' and order.status='ordered'"); 
    
}
else{
$q=mysqli_query($con,"SELECT * FROM  `cart` where status='ordered'"); 

}
// $q=mysqli_query($con,"SELECT * FROM  `cart` inner join order on cart.orderid=order.orderid where cart.status='ordered'"); 

?>
<form action="" method="post">
    <input type="date" id="date1" name="date1" max="<?php echo date('Y-m-d'); ?>">
    <input type="date" id="date2" name="date2" max="<?php echo date('Y-m-d', strtotime('+1 day')); ?>">
